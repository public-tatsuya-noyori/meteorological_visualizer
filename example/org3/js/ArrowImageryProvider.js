...
function ArrowImageryProvider(options) {
  ...
  this._viewers = Cesium.defaultValue(options.viewers, undefined);
...
ArrowImageryProvider.prototype.requestImage = async function (
  ...
  let dataTable = await Arrow.Table.from(fetch([this._urlPrefix, '/', this._datasetPath, '/', level, '/', x, '/', y, '/', this._urlSuffix].join('')));
  let rowNumber = 0;
  for (let row of this._configTable) {
    ...
    let dataFieldNameArray = dataTable.schema.fields.map(field => field.name);
    let latArray = [];
    let lonArray = [];
    let valueArray = [];
    if (dataFieldNameArray.indexOf('latitude [degree]') > -1) {
      latArray = dataTable.getColumn('latitude [degree]').toArray();
    }
    if (dataFieldNameArray.indexOf('longitude [degree]') > -1) {
      lonArray = dataTable.getColumn('longitude [degree]').toArray();
    }
    if (dataFieldNameArray.indexOf(name) > -1) {
      valueArray = dataTable.getColumn(name).toArray();
    }
    if (draw == 'point') {
      let drawCollection = this._viewers[rowNumber].scene.primitives.add(new Cesium.PointPrimitiveCollection());
      ...
      let drawCollection = this._viewers[rowNumber].scene.primitives.add(new Cesium.PolylineCollection());
